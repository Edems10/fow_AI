...
from lol_rules import EXTRA_SUMMONER_SPELL_NAMES, MAP_SIZE, WARD_TYPES
from macro_dataset import MacroDataset
from macro_predictor.macro_predictor import MacroPredictor
    ...
    for epoch in range(1, 6):
        ...
        for name, embedding in (('champion', model.champion_embedding),
            ...
            if embedding:
                ...
                # print(f"{name} embedding vector sizes:")
                # print(f"mean={vector_norm.min().item():.2f}, {vector_norm.mean().item():.2f}, {vector_norm.max().item():.2f}")
...
def visualize_embeddings(model: torch.nn.Module,
                         ...
                         output_folder: str,
                         model_name: str,
                         data_dragon: DataDragon) -> None:
    for name, embedding, names in\
            (('champion', model.champion_embedding, data_dragon.get_champion_names()),
             ('item', model.item_embedding, data_dragon.get_item_names()),
             ('summoner_spell', model.summoner_spell_embedding, data_dragon.get_summoner_spell_names()),
             ('skill', model.skill_embedding, data_dragon.get_champion_skill_names())):
        if embedding:
            filename = os.path.join(output_folder, f'{model_name}_{name}_embeddings.png')
            seen = seen_types[f'{name}_types']  # Embedding seen in the training data
            embeddings = embedding.weight.data[seen].cpu().numpy()  # Skip unknown as it is unseen
            seen_names = [names[i - 1] for i in np.arange(len(seen))[seen]]
            if embeddings.shape[1] == 2:
                fig = px.scatter(x=embeddings[:, 0], y=embeddings[:, 1], text=seen_names, width=1700, height=1700)
            else:
                fig = px.scatter(x=np.zeros(embeddings.shape[0]), y=embeddings[:, 0], text=seen_names, width=300, height=1800)
            fig.update_traces(textposition='top center')
            fig.write_image(filename)
            print(f"Saved {name} embeddings to {filename}")
...
def visualize_predictions(model: torch.nn.Module,
    ...
    position_predictions_folder = os.path.join(output_folder, 'position_predictions')
    position_bad_folder = os.path.join(output_folder, 'position_bad_decisions')
    ...
    champ_state_x_idx, champ_state_y_idx = 31, 32  # TODO: find a way to not break this long-term
...
def main() -> None:
    ...
    for key, value in feature_std.items():
        zero = value == 0
        if zero.any():
            print(f"While normalizing {key} some elements were zero, setting std to 1: {zero}")
            value[zero] = 1
    ...
    features = [
        ...
        'map',
        'champion_state',
        # 'champion_stats', causes overfitting
        'champion_embedding',
        'items',
        'wards',
        'summoner_spells',
    ...
    features_to_ablate = [
        'state', # essential for dragon and rift herald prediction
        'stats', # causes overfitting
        'gameTime', # essential for inhibitor and turret destroyed predictions
        'map',
        'champion_state', # essential for position prediction
        'champion_stats', # causes overfitting
        'champion_embedding',
        'items',
        'wards',
    ...
    targets_to_ablate = list(targets.keys())
    ...
    seen_types = {'champion_types': np.zeros(feature_sizes['champion_embedding'], dtype=bool),
                  'item_types': np.zeros(feature_sizes['item_embedding'], dtype=bool),
                  'summoner_spell_types': np.zeros(feature_sizes['summoner_spell_embedding'], dtype=bool),
                  'skill_types': np.zeros(feature_sizes['skill_embedding'], dtype=bool)}
    for i in range(len(train_dataset)):
        state, champ_idx = train_dataset[i]
        for key in seen_types.keys():
            seen_types[key][state[key]] = True
    macro_predictor = create_model(device,
                                   features,
                                   targets,
                                   feature_sizes,
